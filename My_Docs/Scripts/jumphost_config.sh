...
LOG_DIR="/var/log/bastion/"
aws s3 cp $LOG_DIR s3://ci-prod-bastion/logs/ --sse --region us-east-1 --recursive && find $LOG_DIR* -mtime +1 -exec rm {} \;
...
aws s3api list-objects --bucket ci-prod-bastion --prefix public-keys/ --region us-east-1 --output text --query 'Contents[?Size>`0`].Key' | sed -e 'y/\t/\n/' > ~/keys_retrieved_from_s3
while read line; do
  ...
  if [[ "$USER_NAME" =~ ^[a-z][-a-z0-9]*$ ]]; then
    ...
    if [ -f ~/keys_installed ]; then
      ...
      if [ $? -eq 0 ]; then
        aws s3 cp s3://ci-prod-bastion/$line /home/$USER_NAME/.ssh/authorized_keys --region us-east-1
